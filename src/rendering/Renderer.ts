...
export class Renderer {
    ...
    private _drawEntities(entities: Entity[], selectedIds: Set<number>): void {
        ...
        for (const e of entities) {
            ...
            if (e.type === 'building' || e.type === 'resource') {
                ...
                const dx = r * 2 * ISO.SX;
                const dy = r * 2 * ISO.SY;
                shape.poly([0, -dy, dx, 0, 0, dy, -dx, 0])
                     .fill({ color })
            ...
            if (showHp) {
                ...
                const hpBarY = sc ? -(sc.frameH * sc.scale / 2) - 6 : -r * 2 * ISO.SY - 8;
    ...
    private _drawProjectiles(projectiles: Projectile[]): void {
        ...
        for (const p of projectiles) {
            ...
            g.circle(iso.x, iso.y, 4).fill({ color: hexColor(TEAM_COLORS[p.team] ?? '#ffffff') });
        }
    }
    ...
    private _drawEffects(effects: Effect[]): void {
        ...
        for (const ef of effects) {
            ...
            g.ellipse(iso.x, iso.y, ef.radius * ISO.SX, ef.radius * ISO.SY)
    ...
    private _drawGhost(
        ...
        const dx  = r * 2 * ISO.SX;
        const dy  = r * 2 * ISO.SY;
        g.poly([iso.x, iso.y - dy, iso.x + dx, iso.y, iso.x, iso.y + dy, iso.x - dx, iso.y])
    ...
    private _redrawFog(fog: FogManager): void {
        ...
        for (let cy = 0; cy < fog.fogRows; cy++) {
            for (let cx = 0; cx < fog.fogCols; cx++) {
                ...
                g.poly([p00.x, p00.y, p10.x, p10.y, p11.x, p11.y, p01.x, p01.y])
