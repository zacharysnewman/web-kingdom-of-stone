...
class Game {
            ...
            btn.addEventListener('click', () => {
                ...
                document.querySelectorAll('.diff-btn').forEach(b => {
                    b.className = b.className
                        .replace('bg-blue-700 ring-2 ring-blue-400', 'bg-gray-800')
                        .replace('hover:bg-blue-600', 'hover:bg-gray-700');
                });
                btn.className = btn.className
                    .replace('bg-gray-800', 'bg-blue-700 ring-2 ring-blue-400')
                    .replace('hover:bg-gray-700', 'hover:bg-blue-600');
            });
        });
    ...
    // ── Save / Load (stub — stores minimal snapshot) ──────────────────────────

    _saveState() {
        // Lightweight: just settings so Continue re-runs init with same config
        try {
    ...
    _loadState() {
        ...
        this.aiCount = this.settings.aiCount;
        this.diff    = DIFFICULTY[this.settings.difficulty];
        this.init(false); // fresh game with saved settings
    }
