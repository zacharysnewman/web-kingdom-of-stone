...
export class Game implements IGameContext, IGameAdapter {
  ...
  static async create(): Promise<Game> {
    ...
    app.canvas.style.cssText =
      "position:fixed;top:0;left:0;width:100%;height:100%;touch-action:none;z-index:0;";
    const spriteUrls = [
      ...new Set(
    ...
    await Promise.all([
      ...
      ...tileIndices.map((i) =>
        PIXI.Assets.load({
          src: `tiles/tile_${String(i).padStart(3, "0")}.png`,
  ...
  addEntity(
    ...
    if (
      ...
      this.state.techs.has("soldier_hp")
    ) {
      e.maxHp += 80;
      e.hp = e.maxHp;
    }
    if (type === "building" && this.pathfinding) this.rebuildNavGrid();
    return e;
  }
  ...
  rebuildNavGrid(): void {
    if (this.state.stoneGrid)
      this.pathfinding.rebuild(
        this.state.entities,
        this.state.stoneGrid,
        this.state.mapCellSize,
      );
  }

  trainUnit(building: Entity, type: SubType): boolean {
    if (building.buildQueue.length >= CONSTANTS.MAX_QUEUE) {
      if (building.team === CONSTANTS.TEAM_PLAYER)
        this.notify("Queue full!", "text-orange-400");
      return false;
    }
    const cost = STATS[type].cost;
    if (this.state.gold[building.team] < cost) {
      if (building.team === CONSTANTS.TEAM_PLAYER)
        this.notify("Low Gold!", "text-red-400");
      return false;
    }
  ...
  handleRightClick(wx: number, wy: number): void {
    ...
    const clicked = this.selectionManager.getEntityAt(
      wx,
      wy,
      this.state.entities,
    );
    if (clicked) {
      if (clicked.team !== 0) {
        ...
        this._createEffect(
          clicked.x,
          clicked.y,
          clicked.type === "resource" ? "#fbbf24" : "#ef4444",
        );
      } else if (clicked.isConstructing) {
        units
          .filter((u) => u.subType === "builder")
          .forEach((u) => {
            u.target = clicked;
  ...
  selectAllUnits(): void {
    this.selectionManager.selectAllUnits(
      this.state.entities,
  ...
  private _formationMove(units: Entity[], tx: number, ty: number): void {
    const rows = Math.ceil(Math.sqrt(units.length)),
      spacing = 35;
    const sx = tx - (rows * spacing) / 2,
      sy = ty - (rows * spacing) / 2;
    units.forEach((u, i) => {
  ...
  private _isOnStone(wx: number, wy: number, r: number): boolean {
    ...
    const cols = stoneGrid[0].length,
      rows = stoneGrid.length;
  ...
  private _isBlockedPlacement(wx: number, wy: number, type: SubType): boolean {
    ...
    for (const e of this.state.entities)
      if (
        (e.type === "building" || e.type === "resource") &&
        dist(wx, wy, e.x, e.y) < r + e.radius + 5
      )
        return true;
    return false;
  }
  ...
  private _generateMap(seed: number): void {
    const { stoneGrid, mapCellSize, mines } = generateMap(
      seed,
      this.state.aiCount,
    );
  ...
  private _setupInput(): void {
    this.inputManager.setup(
      ...
      () => ({ width: this._canvasWidth, height: this._canvasHeight }),
      {
        ...
        // Updated to pass this.renderer.worldContainer for screen-space matching
        onBoxSelectUpdate: () => {
          if (this.state.dragSelect) {
            this.selectionManager.performBoxSelect(
              this.state.dragSelect,
              this.state.entities,
              this.state.selectedIds,
              this.renderer.worldContainer, // Required for toGlobal conversion
            );
          }
        },
        ...
        onControlGroupRecall: (digit, doubleTap) => {
          ...
          const alive = ids.filter((id) =>
            this.state.entities.some((en) => en.id === id && !en.isDead),
          );
          this.state.selectedIds = new Set(alive);

          if (doubleTap && this.state.selectedIds.size > 0) {
            const members = this.state.entities.filter((en) =>
              this.state.selectedIds.has(en.id),
            );
            this.camera.x =
              members.reduce((s, en) => s + en.x, 0) / members.length;
            this.camera.y =
              members.reduce((s, en) => s + en.y, 0) / members.length;
            this.camera.clamp();
          }
          this.updateUI();
        },
  ...
  private _handleTap(wx: number, wy: number): void {
    ...
    if (s.placementMode) {
      const sx = Math.round(wx / 20) * 20,
        sy = Math.round(wy / 20) * 20;
      if (s.gold[0] < STATS[s.placementMode].cost) {
        this.notify("Not enough gold!", "text-red-400");
        this.cancelPlacement();
        return;
      }
      if (this._isBlockedPlacement(sx, sy, s.placementMode)) {
        this.notify("Can't build here!", "text-red-400");
        return;
      }
      const bldrs = this._getSelectedEntities().filter(
        (e) => e.subType === "builder",
      );
      if (bldrs.length > 0) {
        s.gold[0] -= STATS[s.placementMode].cost;
        this._syncPlayerStore();
    ...
    if (clicked && clicked.team === 0) {
      if (
        ...
        this._lastClickedId === clicked.id
      ) {
        s.selectedIds.clear();
        s.entities.forEach((e) => {
          if (
            e.team === 0 &&
            e.subType === clicked.subType &&
            dist(e.x, e.y, this.camera.x, this.camera.y) <
              this._canvasWidth / this.camera.zoom
          )
            s.selectedIds.add(e.id);
        });
      } else {
        s.selectedIds.clear();
        s.selectedIds.add(clicked.id);
      }
  ...
  private _update(dt: number): void {
    ...
    if (ex !== 0 || ey !== 0) {
      const wd = isoToWorld(
        (ex * EDGE_SPEED * dt) / this.camera.zoom,
        (ey * EDGE_SPEED * dt) / this.camera.zoom,
      );
      this.camera.x += wd.x;
      this.camera.y += wd.y;
      this.camera.clamp();
    ...
    for (let i = s.entities.length - 1; i >= 0; i--) {
      ...
      if (dead.type === "building") {
        navDirty = true;
        deadBuildingTeams.add(dead.team);
      }
      if (
        dead.type === "unit" &&
        dead.team >= 0 &&
        dead.team < s.population.length
      ) {
  ...
  private _aliveAITCCount(): number {
    return this.state.entities.filter(
      (e) => e.subType === "town_center" && e.team !== 0,
    ).length;
  }
  ...
  private _endGame(win: boolean): void {
    ...
    if (win) {
      ...
      title.className = "text-5xl font-black mb-2 tracking-tight text-blue-400";
      desc.innerText = "All enemies have been crushed.";
    } else {
      title.innerText = "DEFEAT";
      title.className = "text-5xl font-black mb-2 tracking-tight text-red-500";
  ...
  private _refreshTrainingProgress(): void {
    const sel = this._getSelectedEntities();
    const b =
      sel.length === 1 && sel[0].team === 0 && sel[0].type === "building"
        ? sel[0]
        : null;
    this.uiManager.refreshTrainingProgress(
      b,
      this.state.population[0] ?? 0,
      this.state.maxPop[0] ?? 0,
    );
  }

  private _researchTech(key: string, cost: number): void {
    if (this.state.gold[0] < cost) {
      this.notify("Not enough gold!", "text-red-400");
      return;
    }
    ...
    if (key === "soldier_hp") {
      this.state.entities
        ...
        .forEach((e) => {
          e.maxHp += 80;
          e.hp = Math.min(e.hp + 80, e.maxHp);
        });
    }
    const labels: Record<string, string> = {
      mining_efficiency: "Mining Speed",
      archer_damage: "Archer Dmg",
